...
(async () => {
  ...
  const objPesquisa = {
    ...
    nomeCarro : argumento[2],
    anoInicio : argumento[3],
    anoFinal : argumento[4],
    preçoInicial : argumento[5],
    preçoFinal : argumento[6]
  ...
  fs.writeFile(`${objPesquisa.nomeCarro}.txt`, JSON.stringify(carsList, null, 2), err =>{
      if(err) throw new Error('Algo errado não está certo');
